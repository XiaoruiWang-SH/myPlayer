...
播放本地 MP3 文件：无账号、无在线曲库。适合边工作边听音乐、播客或有声书，所有高频操作都能仅靠键盘完成。v1.1 起还能为英文播客自动生成同步转录文稿（可选功能，需自备 Deepgram API 密钥）。
...
v1.1 已发布（2026-08-28）：在 v1.0 基础上新增转录文稿（FR-25~31），PRD §8.1/§8.2 验收全部通过。界面为左右分栏：左侧播放列表、右侧转录文稿面板。`npm run dist` 产出安装包于 `release/`（dmg + zip，arm64 + x64）；应用未签名，首次打开请右键 → 打开。
