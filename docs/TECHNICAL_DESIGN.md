...
| 项目 | 内容 |
| --- | --- |
| 文档版本 | v0.5 草稿（新增 v1.3.1 补丁设计：重新转录二次确认、打包产物清理，2026-08-28 待评审） |
| 日期 | 2026-08-28 |
| 关联文档 | [PRD.md](./PRD.md) |
...
**缓存：** 目录 `userData/transcripts/`。v1.1 缓存键 = `路径 + 大小 + mtime` 拼接后的 SHA-1；**v1.2 起改为曲目稳定 `id`**（渲染层调用 `getTranscript` 时随请求传入），使媒体库导入、重命名都不失效，文件名即 `<id>.json`；删除列表条目时主进程同步删除对应缓存文件。「重新转录」（`force`）跳过读缓存并在成功后覆盖写入；触发前需二次确认（v1.3.1，见下文与 §3.10，FR-41）。
...
**重新转录二次确认（v1.3.1，FR-41）：** 渲染层「重新转录」处理器在调用 `ensureTranscript(force)` 前先走 `confirmAction`（复用 §3.10 确认桥，不新增 IPC）：对话框标题「重新转录」、确认按钮「重新转录」，详情明示会再次消耗 Deepgram 配额并覆盖本地缓存；确认后才发起强制转录，取消则不触发任何请求。
...
**危险操作确认（FR-39）：** 通用确认桥 `confirmAction(options)` → 主进程 `dialog.showMessageBox`（`type: 'question'`，按钮为 `confirmLabel` 与「取消」，**默认按钮为「取消」**），返回 `boolean`。清空列表在执行前先调用，文案明示「列表条目及专属目录中的音频副本都会被删除」；确认后才走清空 + 批量删库内副本流程（批量删除逻辑不变，见 §3.9）。该桥为通用能力，v1.3.1 起同时用于「重新转录」前的确认（§3.8，FR-41）。
...
| 打包产物清理（v1.3.1，FR-42） | `npm run dist` 经 npm 生命周期钩子 `predist` 先执行 `rm -rf release`，再构建打包；产物目录只保留当前版本，避免历次版本累积占用磁盘（历史版本可从 git tag 重新打包） |
...
| M8 v1.3.1 补丁 | 重新转录二次确认、打包前清理旧产物 | FR-41、FR-42 |
